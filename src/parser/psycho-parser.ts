...
import { itienGroups, psychoGroups } from "../constants/groups";
import { addDays } from "date-fns";
import repository from "../db/repository";
import { getPairAndDayByRow } from "../commons/getPairAndDayByRow";

export class PsychoParser extends BaseParser {
  constructor() {
    super(FacultyId.PSYCHO);
  }
  ...
  private async normalizeTable(pathTo: string) {
    ...
    for (const group of psychoGroups) {
  ...
  private async normalizeTableForGroup(
    ...
    for (let r = range.s.r; r <= range.e.r; r++) {
      ...
      if (sheet[cell]) {
        ...
        if (pair) {
          pair.name = sheet[cell].w.replace(/\s+/g, " ");
  ...
  protected getGroupColumn(groupName: string, sheet: Sheet): number {
    ...
    for (let r = range.s.r; r <= range.e.r; r++) {
      for (let c = range.s.c; c <= range.e.c; c++) {
        ...
        if (!sheet[cell]) continue;
        if (
          sheet[cell].w.toLowerCase().split(" ")[0].replace(/\s/g, "") ===
